...
void init_SendscreenSize(){
	...
	ScreenSizepubisher = zmq_socket(context, ZMQ_PUB);
	cv::string port(ScreenSizePort);
	int rc = zmq_bind(ScreenSizepubisher, ("tcp://*:" + port).c_str());
}
...
void Send_ScreenSize(){
	while (true)
	{
		...
		s_send(ScreenSizepubisher, buffer);
		Sleep(1000);
	}
}
...
void Screen()
{
	while (true)
	{
		...
		picindex << picnum;
		string filename = picindex.str() + ".jpg";
